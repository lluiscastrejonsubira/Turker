...
class Server(object):
    ...
    def request(self, operation, parameters = {}):
        ...
        try:
            ...
            print "Number of allowed requests exceeded. Waiting 10s...",
            time.sleep(10) 
            req = urllib2.Request(url = url)
            data = urllib2.urlopen(req)
            print "Sent!"
    ...
    def createhit(self, title, description, page, amount, duration,
        lifetime, maxAssigs=1, keywords = "", autoapprove = 604800, height = 650,
    ...
    def expire(self, hitid):
        """
        Expires a hit early so that no worker can work on it
        """
        r = self.request('ForceExpireHIT',
                         {'HITId': hitid}
                         )
    ...
    def extend(self, hitid, timeInSeconds):
        """
        Extends the lifetime of a hit
        """
        r = self.request('ExtendHIT',
                         {'HITId': hitid,
                          'ExpirationIncrementInSeconds': timeInSeconds}
                         )
        r.validate('ExtendHITResult/Request/IsValid',
                   'ExtendHITResult/Request/Errors/Error/Message')
